...
void draw_text(GLfloat x, GLfloat y, GLfloat z, string text, vector<GLfloat> color)
{
	...
	for (const unsigned char* p = (const unsigned char*)text.c_str(); *p; p++)
...
// Forward declaration for use in mouse callback
...
void mouse(int button, int state, int x, int y)
{
	...
	if (state == GLUT_DOWN)
	{
		...
		else if (button == 3) { mouseWheel(0, 1, x, y); }   // Apple GLUT scroll up
		else if (button == 4) { mouseWheel(0, -1, x, y); }  // Apple GLUT scroll down
	}
...
void initInterface(int argc, char* argv[])
{
	...
	glutMouseFunc(mouse);
	// glutMouseWheelFunc not available in Apple GLUT; handled via mouse callback (buttons 3/4)
	...
	GLuint vertexshader = initshaders(GL_VERTEX_SHADER, SHADERS_DIR "/light.vert.glsl");
	GLuint fragmentshader = initshaders(GL_FRAGMENT_SHADER, SHADERS_DIR "/light.frag.glsl");
